...
def generar_stl(texto_braille):
    ...
    # ── Acumula todas las formas, una sola unión al final ──
    shapes = []

    shapes.append(
        cq.Workplane("XY").box(ancho_base, alto_base, GROSOR_BASE).val()
    )
    ...
    shapes.append(
        cq.Workplane("XY")
        .transformed(offset=(0, -alto_base/2 + margen, cz))
        .text("ECODOT", fontsize=6, distance=0.6, halign="center", valign="center")
        .val()
    )

    modelo = cq.Workplane("XY").newObject([cq.Compound.makeCompound(shapes)])
...
def generar_stl_mapa_con_progreso(contornos, marcadores, leyenda_texto,
    ...
    y_top_mapa    =  alto_base / 2
    y_bot_mapa    =  alto_base / 2 - alto_zona_mapa
    y_top_leyenda =  y_bot_mapa
    y_bot_leyenda = -alto_base / 2
    ...
    # Separador entre zona mapa y zona leyenda
    if n_lineas_ley > 0:
        shapes.append(
            cq.Workplane("XY")
    ...
    progress_cb(71, "Añadiendo marcadores en Braille...")
    for m in marcadores:
    ...
    # ── Leyenda Braille ──
    progress_cb(80, "Añadiendo leyenda Braille...")
    ...
    # ── Marca ECODOT ──
    progress_cb(93, "Añadiendo marca ECODOT...")
    shapes.append(
        cq.Workplane("XY")
